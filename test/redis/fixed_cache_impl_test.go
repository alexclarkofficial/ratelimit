...
func testRedis(usePerSecondRedis bool) func(*testing.T) {
	return func(t *testing.T) {
		...
		limits := []*config.RateLimit{config.NewRateLimit(10, 2, pb.RateLimitResponse_RateLimit_SECOND, sm.NewStats("key_value"), false, false)}

		assert.Equal(
			[]*pb.RateLimitResponse_DescriptorStatus{{Code: pb.RateLimitResponse_OK, CurrentLimit: limits[0].Limit, LimitRemaining: 5, DurationUntilReset: utils.CalculateReset(&limits[0].Limit.Unit, 2, timeSource)}},
		...
		limits = []*config.RateLimit{
			nil,
			config.NewRateLimit(10, 2, pb.RateLimitResponse_RateLimit_MINUTE, sm.NewStats("key2_value2_subkey2_subvalue2"), false, false),
		}
		assert.Equal(
			[]*pb.RateLimitResponse_DescriptorStatus{
				{Code: pb.RateLimitResponse_OK, CurrentLimit: nil, LimitRemaining: 0},
				{Code: pb.RateLimitResponse_OVER_LIMIT, CurrentLimit: limits[1].Limit, LimitRemaining: 0, DurationUntilReset: utils.CalculateReset(&limits[1].Limit.Unit, 2, timeSource)},
			},
			cache.DoLimit(nil, request, limits))
		...
		limits = []*config.RateLimit{
			config.NewRateLimit(10, 2, pb.RateLimitResponse_RateLimit_HOUR, sm.NewStats("key3_value3"), false, false),
			config.NewRateLimit(10, 2, pb.RateLimitResponse_RateLimit_DAY, sm.NewStats("key3_value3_subkey3_subvalue3"), false, false),
		}
		assert.Equal(
			[]*pb.RateLimitResponse_DescriptorStatus{
				{Code: pb.RateLimitResponse_OVER_LIMIT, CurrentLimit: limits[0].Limit, LimitRemaining: 0, DurationUntilReset: utils.CalculateReset(&limits[0].Limit.Unit, 2, timeSource)},
				{Code: pb.RateLimitResponse_OVER_LIMIT, CurrentLimit: limits[1].Limit, LimitRemaining: 0, DurationUntilReset: utils.CalculateReset(&limits[1].Limit.Unit, 2, timeSource)},
			},
			cache.DoLimit(nil, request, limits))
		assert.Equal(uint64(1), limits[0].Stats.TotalHits.Value())
...
func TestOverLimitWithLocalCache(t *testing.T) {
	...
	limits := []*config.RateLimit{
		config.NewRateLimit(15, 2, pb.RateLimitResponse_RateLimit_HOUR, sm.NewStats("key4_value4"), false, false),
	}

	assert.Equal(
		[]*pb.RateLimitResponse_DescriptorStatus{
			{Code: pb.RateLimitResponse_OK, CurrentLimit: limits[0].Limit, LimitRemaining: 4, DurationUntilReset: utils.CalculateReset(&limits[0].Limit.Unit, 2, timeSource)},
		},
		cache.DoLimit(nil, request, limits))
	assert.Equal(uint64(1), limits[0].Stats.TotalHits.Value())
	assert.Equal(uint64(0), limits[0].Stats.OverLimit.Value())
	...
	assert.Equal(
		[]*pb.RateLimitResponse_DescriptorStatus{
			{Code: pb.RateLimitResponse_OK, CurrentLimit: limits[0].Limit, LimitRemaining: 2, DurationUntilReset: utils.CalculateReset(&limits[0].Limit.Unit, 2, timeSource)},
		},
		cache.DoLimit(nil, request, limits))
...
func TestNearLimit(t *testing.T) {
	...
	limits = []*config.RateLimit{config.NewRateLimit(20, 2, pb.RateLimitResponse_RateLimit_SECOND, sm.NewStats("key5_value5"), false, false)}

	assert.Equal(
		[]*pb.RateLimitResponse_DescriptorStatus{{Code: pb.RateLimitResponse_OK, CurrentLimit: limits[0].Limit, LimitRemaining: 15, DurationUntilReset: utils.CalculateReset(&limits[0].Limit.Unit, 2, timeSource)}},
		cache.DoLimit(nil, request, limits))
	assert.Equal(uint64(3), limits[0].Stats.TotalHits.Value())
	assert.Equal(uint64(0), limits[0].Stats.OverLimit.Value())
	assert.Equal(uint64(0), limits[0].Stats.NearLimit.Value())
	...
	limits = []*config.RateLimit{config.NewRateLimit(8, 2, pb.RateLimitResponse_RateLimit_SECOND, sm.NewStats("key6_value6"), false, false)}

	assert.Equal(
		[]*pb.RateLimitResponse_DescriptorStatus{{Code: pb.RateLimitResponse_OK, CurrentLimit: limits[0].Limit, LimitRemaining: 1, DurationUntilReset: utils.CalculateReset(&limits[0].Limit.Unit, 2, timeSource)}},
		cache.DoLimit(nil, request, limits))
	assert.Equal(uint64(2), limits[0].Stats.TotalHits.Value())
	assert.Equal(uint64(0), limits[0].Stats.OverLimit.Value())
	assert.Equal(uint64(1), limits[0].Stats.NearLimit.Value())
	...
	limits = []*config.RateLimit{config.NewRateLimit(20, 2, pb.RateLimitResponse_RateLimit_SECOND, sm.NewStats("key7_value7"), false, false)}
	...
	limits = []*config.RateLimit{config.NewRateLimit(20, 2, pb.RateLimitResponse_RateLimit_SECOND, sm.NewStats("key8_value8"), false, false)}

	assert.Equal(
		[]*pb.RateLimitResponse_DescriptorStatus{{Code: pb.RateLimitResponse_OVER_LIMIT, CurrentLimit: limits[0].Limit, LimitRemaining: 0, DurationUntilReset: utils.CalculateReset(&limits[0].Limit.Unit, 2, timeSource)}},
		cache.DoLimit(nil, request, limits))
	assert.Equal(uint64(3), limits[0].Stats.TotalHits.Value())
	assert.Equal(uint64(2), limits[0].Stats.OverLimit.Value())
	assert.Equal(uint64(1), limits[0].Stats.NearLimit.Value())
	...
	limits = []*config.RateLimit{config.NewRateLimit(20, 2, pb.RateLimitResponse_RateLimit_SECOND, sm.NewStats("key9_value9"), false, false)}
	...
	limits = []*config.RateLimit{config.NewRateLimit(10, 2, pb.RateLimitResponse_RateLimit_SECOND, sm.NewStats("key10_value10"), false, false)}
...
func TestOverLimitWithLocalCacheShadowRule(t *testing.T) {
	...
	limits := []*config.RateLimit{
		config.NewRateLimit(15, 2, pb.RateLimitResponse_RateLimit_HOUR, sm.NewStats("key4_value4"), false, true),
	}
	...
	assert.Equal(
		[]*pb.RateLimitResponse_DescriptorStatus{
			{Code: pb.RateLimitResponse_OK, CurrentLimit: limits[0].Limit, LimitRemaining: 0, DurationUntilReset: utils.CalculateReset(&limits[0].Limit.Unit, 2, timeSource)},
		},
		cache.DoLimit(nil, request, limits))
	assert.Equal(uint64(3), limits[0].Stats.TotalHits.Value())
	assert.Equal(uint64(1), limits[0].Stats.OverLimit.Value())
	...
	assert.Equal(
		[]*pb.RateLimitResponse_DescriptorStatus{
			{Code: pb.RateLimitResponse_OK, CurrentLimit: limits[0].Limit, LimitRemaining: 15, DurationUntilReset: utils.CalculateReset(&limits[0].Limit.Unit, 2, timeSource)},
		},
		cache.DoLimit(nil, request, limits))
