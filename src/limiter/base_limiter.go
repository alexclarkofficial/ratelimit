	...
	if isOverLimitWithLocalCache {
		...
		if limitInfo.limitAfterIncrease > limitInfo.overLimitThreshold {
			...
			if this.localCache != nil {
				...
				err := this.localCache.Set([]byte(key), []byte{}, int(utils.UnitToDivider(limitInfo.limit.Limit.Unit)*int64(limitInfo.limit.Limit.UnitMultiplier)))
				if err != nil {
...
func (this *BaseRateLimiter) generateResponseDescriptorStatus(responseCode pb.RateLimitResponse_Code,
	...
	if limit != nil {
		return &pb.RateLimitResponse_DescriptorStatus{
			...
			DurationUntilReset: utils.CalculateReset(&limit.Unit, limit.UnitMultiplier, this.timeSource),
		}
	} else {
		return &pb.RateLimitResponse_DescriptorStatus{
			Code:           responseCode,
